...
int LayeredProvingTree::LPTNode::nodeCount = -1;

QString TruthValueNames[] = {"FALSE", "TRUE", "UNKNOWN"};
...
void LayeredProvingTree::addResultsToModel(QStandardItemModel *model)
{
    QString label;
    int digits = std::log10(assumptions.count()) + 1;

    // add assumptions to model
    QStandardItem *parent = model->item(0);
    for (int i=0; i<assumptions.count(); ++i) {
        QTextStream(&label) << "(A" << qSetFieldWidth(digits) << i+1 << qSetFieldWidth(0) << "): ";
        parent->appendRow(new QStandardItem(label + assumptions.at(i)->getString()));
        label.clear();
    }

    // add steps to model
    digits = std::log10(steps.count()) + 1;
    parent = model->item(1);
    for (int i=0; i<steps.count(); ++i) {
        QTextStream(&label) << "(S" << qSetFieldWidth(digits) << i+1 << qSetFieldWidth(0) << "): ";
        parent->appendRow(new QStandardItem(label + steps.at(i)->getString()));
        label.clear();
    }
    ...
    parent = model->item(2);
    for (int i=0; i<root.children.count(); ++i) {
        root.children.at(i)->addResultToModel(parent);
    }
...
LayeredProvingTree::LayeredProvingTree(ImplementedLogics logic, StatementPtrList &assumptions,
                                       StatementPtrList &steps, StatementPtrList &goals, QTextBrowser *comment)
    :allNodesTrueClosed(false)
{
    ...
    case ImplementedLogics::GNY:
        ...
        break;
    case ImplementedLogics::BAN:
        l = new BANLogic::BANLogicImpl(commentFrame);
        l->setLPT(this);
        break;
    default:
        l = nullptr;
...
bool LayeredProvingTree::verifyProtocol(bool exhaustive, TraverseType strategy)
{
    try {
        ...
        QMessageBox::critical(nullptr, "Cryptographic-Protocol Development & Verification Tool", QString("bad_cast Exception ") + ex.what());
    } catch (std::exception ex) {
        QMessageBox::critical(nullptr, "Cryptographic-Protocol Development & Verification Tool", ex.what());
    } catch (...) {
...
bool LayeredProvingTree::expand(bool exhaustive, TraverseType strategy)
{
    ...
    // if nextNode == nullptr
    if (nextNode == nullptr || nextNode->isRoot) {
        ...
        for (int i=0; i<matchingPostList.count(); ++i) {
            ...
            // make sure instantiatedPostList is empty
            instantiatedPostList.clear();
            // logic->getAllInstantiatedPostulates(p, instantiatedPostList)
    ...
    case TraverseType::DFS:
        ...
        while (!nodeStack.isEmpty()) {
            ...
            if (current->layerType == LinkType::ANDLink) {
                ...
                // if (current is suitable for expansion) - Not Root && TruthValue=Unknown && !isExtended && !violatesTermination
                if (!current->isRoot && current->truthValue==TruthValue::UNKNOWN && !current->isExtended && !current->violatesTerminationRules) {
                    nextNode = current;
                    break;
                } else {
...
void LayeredProvingTree::LPTNode::addResultToModel(QStandardItem *parent)
{
    QStandardItem *newItem;
    QString label;
    QString pre = (this->parent == &(lpt->root))?"(G":"(";
    int digits = std::log10(nodeCount) + 1;
    int gdigits = std::log10(lpt->goals.count());
    if (this->parent == &(lpt->root)) {
        // this is a goal
        QTextStream(&label) << pre << qSetFieldWidth(gdigits) << nodeID << qSetFieldWidth(0) << "): ";
    } else {
        QTextStream(&label) << pre << qSetFieldWidth(digits) << nodeID << qSetFieldWidth(0) << "): ";
    }

    if (layerType == LinkType::ANDLink) {
        // node stores statement
        newItem = new QStandardItem(label + stmt->getString() + " has truth value " +
                                    TruthValueNames[static_cast<int>(truthValue)]);
        parent->appendRow(newItem);
        for (int i=0; i<children.count(); ++i) {
            children.at(i)->addResultToModel(newItem);
        }
    } else if (layerType == LinkType::ORLink) {
        // node stores postulate
        newItem = new QStandardItem(label + postulateDetail + " - has truth value " +
                                    TruthValueNames[static_cast<int>(truthValue)]);
        parent->appendRow(newItem);
        for (int i=0; i<children.count(); ++i) {
            children.at(i)->addResultToModel(newItem);
        ...
        throw LPTException("Unknown link type in LPT::LPTNode::addResultsToModel");
    }
}
...
LayeredProvingTree::LPTAndNode::LPTAndNode(LPTNode *nodeParent, Statement *nodeStmt)
{
    ...
    if (crossRefTarget != nullptr) {
        ...
        // check if node is in sigma
        // nodes are in sigma but don't have a crossRefTarget is stmt is either initial assumption or protocol step
        if (lpt->sigma.containsPtrToEquivalentObject(stmt)) {
...
TruthValue operator||(TruthValue first, TruthValue second)
{
    ...
    std::cout << TruthValueNames[static_cast<int>(first)].toStdString() << " || "  << TruthValueNames[static_cast<int>(second)].toStdString()
                                                                        << " = " << TruthValueNames[static_cast<int>(result)].toStdString() << std::endl;
    return result;
}
